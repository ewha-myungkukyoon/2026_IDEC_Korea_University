 ...
 *       Filename:  main.cu
 *
 *    Description:  Blurs an image by averaging each pixel's neighborhood,
 *                  computed on both the CPU and GPU, and compares the
...
//MK: Averages each pixel's neighborhood on the GPU, one thread per pixel
__global__
