 ...
 *       Filename:  main.cu
 *
 *    Description:  Exercise: add shared-memory caching to the tiled gemm()
 *                  from 06_simpleCNNBatchTiling, which already organizes
 *                  the GEMM into 16x16 output tiles but still reads from
 *                  global memory. Compare Conv1's timing against 06's.
 *                  Work in layer.h - this file needs no changes.
 *
 *        Version:  1.0
...
int main(){
	...
	clockMeasure *ckGemm = new clockMeasure("GEMM");
	...
	for(int batchStart = 0; batchStart < testCount; batchStart += BATCH_SIZE){
		...
		runIm2col(h_batchImages, batchSize);
		ckIm2col->clockPause();
		ckGemm->clockResume();
		runGemm(h_conv1W, h_conv1Bias, h_batchConv1Out, batchSize);
		ckGemm->clockPause();
		ckConv->clockPause();
	...
	printf("\t\t- "); ckIm2col->clockPrint();
	printf("\t\t- "); ckGemm->clockPrint();
