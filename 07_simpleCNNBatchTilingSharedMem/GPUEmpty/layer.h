 ...
 *       Filename:  layer.h
 *
 *    Description:  Exercise: add shared-memory caching to 06's tiled
 *                  gemm(). Same 2D block/grid mapping, same im2col(). Per
 *                  K chunk every thread loads exactly ONE element into
 *                  each of two __shared__ tiles, __syncthreads(), does
 *                  GEMM_TILE multiply-adds from shared, then syncs again.
 *
 *        Version:  1.0
...
//MK: TODO - shared-memory-tiled GEMM; see matmulCpu above for the reference math.
...
//MK: Cooperatively load the whole weight matrix and this tile's im2col columns into __shared__ arrays once per block
//MK: (__syncthreads() in between), then have each thread compute one (channel, position) dot product purely from shared memory.
__global__
