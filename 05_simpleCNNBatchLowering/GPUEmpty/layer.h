 ...
 *       Filename:  layer.h
 *
 *    Description:  Exercise: implement Conv1 as im2col + a plain GEMM -
 *                  no tiling, no shared memory, flat 1D indexing (one
 *                  thread per batch/channel/position). im2colCpu and
 *                  matmulCpu are the reference. USE_GPU_CONV1 turns both
 *                  kernels on at once; ReLU/Pool/FC are unchanged from 04.
 *
 *        Version:  1.0
 *        Created:  07/21/2026 10:00:00 AM
 *       Revision:  none
 *       Compiler:  nvcc
 *
...
//MK: TODO - plain GEMM, no shared memory; see matmulCpu above for the reference math.
//MK: One thread per (batch, output channel, output position) - batchSize*CONV1_OUT_SIZE total, same as
//MK: conv1Forward in 04_simpleCNNBatch - each thread reads its own 27-element weight row and im2col
//MK: column straight from global memory (relying on L1 for reuse) and writes one output element.
__global__
...
void runGemm(const float *h_conv1W, const float *h_conv1Bias, float *h_output, int batchSize){
	if(USE_GPU_CONV1){
		gemm<<<numBlocks(batchSize * CONV1_OUT_SIZE), THREADS>>>(d_bufIm2col, d_conv1W, d_conv1Bias, d_bufConvOut, batchSize);
