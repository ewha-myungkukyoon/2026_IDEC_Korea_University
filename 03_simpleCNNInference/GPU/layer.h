 ...
 *       Filename:  layer.h
 *
 *    Description:  Conv/ReLU/Pool/FC layers for the simple CIFAR-10 CNN:
 *                  a CPU reference and a GPU kernel for each, plus a
 *                  runXxx() wrapper that dispatches to one or the other
 *                  per USE_GPU_* toggle. All toggles are on here, so the
 *                  whole forward pass runs on the GPU.
 *
 *        Version:  1.0
...
//MK: ==================== GPU kernels, each matching its CPU counterpart above ====================
...
//MK: 2x2 MaxPool forward, one thread per pooled output element
__global__
...
//MK: FC1 forward, one thread per output class (Flatten is implicit - pool output is already a contiguous 3600 vector)
__global__
