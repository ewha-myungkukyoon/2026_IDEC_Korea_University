 ...
 *       Filename:  main.cu
 *
 *    Description:  Exercise: reshape 05's flat gemm() into a square-tiled
 *                  grid/block - no shared memory yet, just the
 *                  decomposition. Each thread does the same dot product,
 *                  but the smaller per-block working set lets L1 pick up
 *                  part of the win. Work in layer.h - no changes here.
 *
 *        Version:  1.0
