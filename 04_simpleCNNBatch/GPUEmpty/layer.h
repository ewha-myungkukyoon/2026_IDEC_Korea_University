 ...
 *       Filename:  layer.h
 *
 *    Description:  Exercise: port the BATCHED CIFAR-10 CNN forward pass
 *                  to the GPU one layer at a time. Every function takes a
 *                  batchSize, so one launch covers the whole batch. Each
 *                  CPU reference is written - implement the kernel next to
 *                  it and flip its USE_GPU_* toggle on, in order.
 *
 *        Version:  1.0
...
//MK: TODO - one thread per (batch, output element) pair (batchSize*CONV1_OUT_SIZE total); see conv1ForwardCpu above
__global__
