 ...
 *       Filename:  main.cu
 *
 *    Description:  Full BATCHED GPU inference for the simple CIFAR-10
 *                  CNN: Conv1 -> ReLU -> MaxPool -> FC1, computed
 *                  BATCH_SIZE images per kernel launch instead of one at
 *                  a time. Loads pre-trained weights and reports Top-1
 *                  accuracy and per-layer timing on the test set.
 *
 *        Version:  1.0
